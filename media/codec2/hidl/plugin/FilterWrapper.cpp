...
class PassingListener : public C2Component::Listener {
public:
    PassingListener(
...
class LastListener : public C2Component::Listener {
public:
    LastListener(
            std::shared_ptr<C2Component> wrappedComponent,
            const std::shared_ptr<Listener> &wrappedComponentListener)
...
class WrappedEncoderInterface : public C2ComponentInterface {
public:
    WrappedEncoderInterface(
            std::shared_ptr<C2ComponentInterface> intf,
            std::vector<FilterWrapper::Component> &&filters,
            std::weak_ptr<FilterWrapper> filterWrapper)
        : mIntf(intf), mFilterWrapper(filterWrapper) {
    ...
    ~WrappedEncoderInterface() override = default;
    ...
    void takeFilters(std::vector<FilterWrapper::Component> &&filters) {
        ...
        for (size_t i = 0; i < mFilters.size(); ++i) {
            if (i == mFilters.size() -1) {
                transferParams_l(mFilters[i].intf, mIntf, C2_MAY_BLOCK);
            } else {
                transferParams_l(mFilters[i].intf, mFilters[i + 1].intf, C2_MAY_BLOCK);
            }
            for (C2Param::Type type : mFilters[i].desc.controlParams) {
                mTypeToIndexForQuery[type.type()] = i;
                mTypeToIndexForConfig[type.type() & ~C2Param::CoreIndex::IS_REQUEST_FLAG] = i;
        ...
        for (size_t i = 0; i < mFilters.size(); ++i) {
            if (i == mFilters.size() -1) {
                backPropagateParams_l(mFilters[mFilters.size() - 1].intf, mIntf, C2_MAY_BLOCK);
            } else {
                backPropagateParams_l(mFilters[i].intf, mFilters[i + 1].intf, C2_MAY_BLOCK);
            }
        }
    }
    ...
    c2_status_t config_vb(
        ...
        for (size_t i = 0; i < mFilters.size(); ++i) {
            ...
            if (err != C2_OK) {
                LOG(err == C2_BAD_INDEX ? VERBOSE : WARNING)
                        << "WrappedEncoderInterface: " << filter->getName()
                        << " returned error for config_vb; err=" << err;
                result = err;
            }

            if (i == mFilters.size() -1) {
                transferParams_l(mFilters[i].intf, mIntf, mayBlock);
            } else {
                transferParams_l(mFilters[i].intf, mFilters[i + 1].intf, mayBlock);
            }
        ...
        if (err != C2_OK) {
            LOG(err == C2_BAD_INDEX ? VERBOSE : WARNING)
                    << "WrappedEncoderInterface: " << mIntf->getName()
                    << " returned error for config_vb; err=" << err;
            result = err;
        }
    ...
    c2_status_t querySupportedParams_nb(
        ...
        c2_status_t result = mIntf->querySupportedParams_nb(params);
        if (result != C2_OK) {
            LOG(WARNING) << "WrappedEncoderInterface: " << mIntf->getName()
                    << " returned error for querySupportedParams_nb; err=" << result;
            return result;
        }
        ...
        for (const FilterWrapper::Component &filter : mFilters) {
            ...
            c2_status_t err = filter.intf->querySupportedParams_nb(&filterParams);
            if (err != C2_OK) {
                LOG(WARNING) << "WrappedEncoderInterface: " << filter.intf->getName()
                        << " returned error for querySupportedParams_nb; err=" << result;
                result = err;
                continue;
            }
...
c2_status_t transferParams_l(
    ...
    c2_status_t err = next->querySupportedParams_nb(&paramDescs);
    if (err != C2_OK) {
        LOG(DEBUG) << "WrappedEncoderInterface: " << next->getName()
                << " returned error for querySupportedParams_nb; err=" << err;
        return err;
    }
    // Find supported input params from the next interface and flip direction
    ...
    for (const std::shared_ptr<C2ParamDescriptor> &paramDesc : paramDescs) {
        C2Param::Index index = paramDesc->index();
        if (!index.forOutput() || paramDesc->isReadOnly()) {
            continue;
        }
        if (index.forStream()) {
            uint32_t stream = index.stream();
            index = index.withPort(false /* output */).withStream(stream);
        } else {
            index = index.withPort(false /* output */);
        }
        indices.push_back(index);
    }
    // Query those output params from the current interface
    ...
    err = curr->query_vb({}, indices, mayBlock, &heapParams);
    if (err != C2_OK && err != C2_BAD_INDEX) {
        LOG(DEBUG) << "WrappedEncoderInterface: " << curr->getName()
                << " returned error for query_vb; err=" << err;
        return err;
    }
    // Flip the direction of the queried params, so they become input parameters.
...
c2_status_t backPropagateParams_l(
    ...
    //send the params from encoder pre filter queryParamsForPreviousComponent()
    //to next component.
    std::shared_ptr<FilterWrapper> filterWrapper = mFilterWrapper.lock();
    if (!filterWrapper) {
        LOG(DEBUG) << "WrappedEncoderInterface: FilterWrapper not found";
        return C2_OK;
    }
    if (!filterWrapper->isFilteringEnabled(curr)) {
        LOG(VERBOSE) << "WrappedEncoderInterface: filtering not enabled";
        return C2_OK;
    }
    std::vector<std::unique_ptr<C2Param>> params;
    c2_status_t err = filterWrapper->queryParamsForPreviousComponent(curr, &params);
    if (err != C2_OK) {
        LOG(DEBUG) << "WrappedEncoderInterface: FilterWrapper returned error for "
            << "queryParamsForPreviousComponent; intf=" << curr->getName() << " err=" << err;
        return C2_OK;
    }
    ...
    std::vector<std::unique_ptr<C2SettingResult>> failures;
    next->config_vb(configParams, mayBlock, &failures);
    if (err != C2_OK && err != C2_BAD_INDEX) {
        LOG(DEBUG) << "WrappedEncoderInterface: " << next->getName()
                << " returned error for config_vb; err=" << err;
        return err;
...
class WrappedEncoder : public C2Component, public std::enable_shared_from_this<WrappedEncoder> {
public:
    WrappedEncoder(
            std::shared_ptr<C2Component> comp,
            std::vector<FilterWrapper::Component> &&filters,
        ...
        : mComp(comp), mFilters(std::move(filters)), mFilterWrapper(filterWrapper) {
        std::vector<FilterWrapper::Component> filtersDup(mFilters);
        mIntf = std::make_shared<WrappedEncoderInterface>(
                comp->intf(), std::move(filtersDup), filterWrapper);
    }

    ~WrappedEncoder() override = default;
    ...
    c2_status_t queue_nb(std::list<std::unique_ptr<C2Work>>* const items) override {
        ...
        if(!mRunningFilters.empty()){
            return mRunningFilters.front().comp->queue_nb(items);
        }
    ...
    c2_status_t drain_nb(drain_mode_t mode) override {
        ...
        case DRAIN_COMPONENT_WITH_EOS: {
            ...
            items.push_back(std::move(eosWork));
            if(!mRunningFilters.empty()){
                mRunningFilters.front().comp->queue_nb(&items);
                return C2_OK;
            }
...
private:
    ...
    std::shared_ptr<C2Component> mComp;
    std::shared_ptr<WrappedEncoderInterface> mIntf;
    std::vector<FilterWrapper::Component> mFilters;
    std::vector<FilterWrapper::Component> mRunningFilters;
    std::weak_ptr<FilterWrapper> mFilterWrapper;
    std::shared_ptr<Listener> mListener;
    ...
    c2_status_t setListenerInternal(
        ...
        filters.back().comp->setListener_vb(passingListener, mayBlock);
...
std::vector<FilterWrapper::Component> FilterWrapper::createFilters() {
    ...
    for (const FilterWrapper::Component &filter : mComponents) {
        std::shared_ptr<C2Component> comp;
        std::shared_ptr<C2ComponentInterface> intf;
        if(filter.traits.name.find("prefil") != std::string::npos)
            continue;
...
//decoder filters and encoder pre filters are in c2store.
//we do not want to create them together,
//so use name to distinguish them
std::vector<FilterWrapper::Component> FilterWrapper::createPreFilters() {
    std::vector<FilterWrapper::Component> filters;
    for (const FilterWrapper::Component &filter : mComponents) {
        std::shared_ptr<C2Component> comp;
        std::shared_ptr<C2ComponentInterface> intf;
...
std::shared_ptr<C2ComponentInterface> FilterWrapper::maybeWrapInterface(
    ...
    else if(traits.domain == C2Component::DOMAIN_VIDEO && traits.kind == C2Component::KIND_ENCODER){
        return std::make_shared<WrappedEncoderInterface>(
                intf, createPreFilters(), weak_from_this());
    }else
        return intf;
...
std::shared_ptr<C2Component> FilterWrapper::maybeWrapComponent(
    ...
    if((traits.domain == C2Component::DOMAIN_VIDEO || traits.domain == C2Component::DOMAIN_IMAGE)
        ...
        std::vector<Component> filters = createPreFilters();
        std::shared_ptr wrapped = std::make_shared<WrappedEncoder>(
                comp, std::vector(filters), weak_from_this());
        {
            std::unique_lock lock(mWrappedComponentsMutex);
            std::vector<std::weak_ptr<const C2Component>> &components =
